...
const ChatWidget = () => {
  ...
  // Reconnect scheduling refs (avoid stale state in timeouts/closures)
  ...
  // Prevent immediate duplicate (same text & sender) messages
  const addMessage = (text, sender = "user") => {
    ...
    setMessages((prev) => {
      ...
      if (last && last.text === text && last.sender === sender) {
        // don't add duplicate consecutive message
        return prev;
      }
      added = true;
    ...
    return added ? newMessage : null;
  };

  const handleSendMessage = async (messageText = inputValue) => {
    if (!messageText.trim()) return;

    // Build user message and add it synchronously to UI (avoid relying on state update timing)
    const userMessage = {
      id: Date.now() + Math.random(),
      text: messageText,
      sender: "user",
    ...
    try {
      // Pass conversation including just-added user message to the service
      ...
      setTimeout(() => {
        ...
        if (response.success) {
          // If a reconnect was scheduled, cancel it — we've recovered
          if (reconnectScheduledRef.current && reconnectTimeoutRef.current) {
            clearTimeout(reconnectTimeoutRef.current);
            reconnectTimeoutRef.current = null;
            reconnectScheduledRef.current = false;
          }
          ...
          // Add the error message from the API (if it's not an immediate duplicate)
          addMessage(response.message, "bot");
          setIsConnected(false);

          // Schedule a single "I'm back online" message (only one scheduled at a time)
          if (!reconnectScheduledRef.current) {
            reconnectScheduledRef.current = true;
            reconnectTimeoutRef.current = setTimeout(() => {
              addMessage(
                "I'm back online! 🟢 Feel free to continue our conversation or reach out to our team directly at hello@visqode.com",
                "bot"
              );
              setIsConnected(true);
              reconnectScheduledRef.current = false;
      ...
      }, 1000 + Math.random() * 1000); // 1-2s simulated delay
    } catch (error) {
      // In catch case, avoid scheduling multiple reconnect messages and avoid duplicates
      setIsTyping(false);
      setIsConnected(false);
      addMessage(
        "I'm experiencing some technical difficulties. Please try again in a moment, or contact our team directly at hello@visqode.com for immediate assistance.",
        "bot"
      );
  ...
  const handleQuickReply = (reply) => {
    ...
    setQuickReplies([]); // Clear quick replies after use
  };
  ...
  const getConnectionStatus = () => {
    ...
    return { color: "bg-green-500", text: "AI Assistant Online" };
  };
    ...
    <>
      ...
      >
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          className="w-16 h-16 bg-[#a7ff59] rounded-full shadow-lg flex items-center justify-center text-black hover:bg-[#8fee3f] transition-all duration-300 relative"
        >
          ...
          >
            ...
            <div className="bg-gradient-to-r from-[#a7ff59] to-[#8fee3f] p-4 flex items-center justify-between">
              <div className="flex items-center">
                <div className="w-10 h-10 bg-black rounded-full flex items-center justify-center mr-3 relative">
                  <span className="text-[#a7ff59] racing font-bold">V</span>
                  <motion.div
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((message, index) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <p className="openSans text-sm leading-relaxed">
                        {message.text}
                      </p>
                    </div>
                  ...
                  {message.sender === "bot" && (
                    <div className="w-8 h-8 bg-gradient-to-r from-[#a7ff59] to-[#8fee3f] rounded-full flex items-center justify-center mr-2 flex-shrink-0 order-1 relative">
                      <span className="text-black racing font-bold text-sm">
                        V
                      </span>
                      <div className="absolute -top-1 -right-1 w-3 h-3 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full"></div>
                    </div>
                  )}
                </motion.div>
              ))}
              ...
              {isTyping && (
                ...
                >
                  <div className="w-8 h-8 bg-gradient-to-r from-[#a7ff59] to-[#8fee3f] rounded-full flex items-center justify-center mr-2 flex-shrink-0 relative">
                    <span className="text-black racing font-bold text-sm">
                      V
                    </span>
                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full"></div>
                  ...
                  <div className="bg-gradient-to-r from-gray-100 to-gray-50 p-3 rounded-2xl border border-gray-200">
                    <div className="flex space-x-1">
                      <motion.div
                        ...
                        className="w-2 h-2 bg-[#a7ff59] rounded-full"
                      />
                      <motion.div
                        animate={{ scale: [1, 1.2, 1] }}
                        transition={{
                ...
                <div className="grid grid-cols-1 gap-2">
                  {quickReplies.slice(0, 3).map((reply, index) => (
                    <motion.button
                      ...
                      className="p-3 bg-white hover:bg-[#a7ff59]/10 hover:border-[#a7ff59] border border-gray-200 rounded-xl text-xs openSans text-left transition-all duration-300 flex items-center"
                    >
            ...
            <div className="p-4 border-t border-gray-100 bg-white">
              <div className="flex items-center space-x-2">
                ...
                <motion.button
                  ...
                  className="w-10 h-10 bg-[#a7ff59] rounded-xl flex items-center justify-center text-black hover:bg-[#8fee3f] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isTyping ? (
                    <motion.div
                      animate={{ rotate: 360 }}
