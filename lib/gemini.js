...
// Initialize Gemini AI (keep your real key in envs for production)
...
class GeminiService {
  ...
  // Helper: safe extraction of text from the SDK response object.
  ...
  async generateResponse(userMessage, conversationHistory = []) {
    try {
      ...
      const contextMessages = conversationHistory
        ...
        .map(
          (msg) =>
            `${msg.sender === "user" ? "User" : "VisQode Assistant"}: ${
              msg.text
            }`
        )
        .join("\n");
...
VisQode Assistant: `;
      ...
      const possibleResponse =
        result?.response ?? result?.outputs?.[0] ?? result ?? null;
...
Please respond immediately to assist this user.`,
  ...
  getFallbackResponse(userMessage) {
    ...
    if (isUrgent) {
      return `I understand this is urgent! For immediate assistance, please contact our team directly at ibwmahin@gmail.com or call +1 (555) 123-4567. Our team will respond to urgent matters within 2 hours.`;
    }
